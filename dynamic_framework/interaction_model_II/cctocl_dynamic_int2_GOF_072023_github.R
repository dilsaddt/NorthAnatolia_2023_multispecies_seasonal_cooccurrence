...
cat(file = "cctocl_dynamic_int2_072023_gof.txt"," 
model {

# Model: cctocl_dynamic_int2_072023_gof
#
...
cctocl_dynamic_int2_072023_gof <- jags(jags.data, inits, params, "cctocl_dynamic_int2_072023_gof.txt", n.chains = nc, n.adapt = na, n.iter = ni, n.burnin = nb, n.thin = nt, parallel = T)

end <- Sys.time()

end - start

print(cctocl_dynamic_int2_072023_gof)

str(cctocl_dynamic_int2_072023_gof$sims.list)
...
for (i in 1:cctocl_dynamic_int2_072023_gof$mcmc.info$n.samples){
  yrep.cl[[i]] <- cctocl_dynamic_int2_072023_gof$sims.list$yrep.cl[i,]
...
for (i in 1:cctocl_dynamic_int2_072023_gof$mcmc.info$n.samples){
  p_model[[i]] <- cctocl_dynamic_int2_072023_gof$sims.list$pstar[i,]
...
y3drep <- vector(mode = 'list', length = cctocl_dynamic_int2_072023_gof$mcmc.info$n.samples)

for (k in 1:cctocl_dynamic_int2_072023_gof$mcmc.info$n.samples){
...
p_est <- vector(mode = 'list', length = cctocl_dynamic_int2_072023_gof$mcmc.info$n.samples)
...
for (i in 1:cctocl_dynamic_int2_072023_gof$mcmc.info$n.samples){
  psi.cl[[i]] <- cctocl_dynamic_int2_072023_gof$sims.list$psi.cl[i,,]
  gamma.cl[[i]] <- cctocl_dynamic_int2_072023_gof$sims.list$gamma.cl[i,,]
  eps.cl[[i]] <- cctocl_dynamic_int2_072023_gof$sims.list$eps.cl[i,,]

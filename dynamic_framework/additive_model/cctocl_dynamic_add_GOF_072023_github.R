...
cat(file = "cctocl_dynamic_add_072023_gof.txt"," 
model {

# Model: cctocl_dynamic_add_072023_gof
#
...
for (i in 1:nsite){
  for (s in 2:nprimary){
    ...
    + betagamma.cc[1]*season[i,s-1] + betagamma.cc[2]*popden[i] + betagamma.cc[3]*elevation[i]
    + randomgamma.cc[area[i]]
    
    logit(eps.cc[i,s-1]) <- alphaeps.cc 
    + betaeps.cc[1]*season[i,s-1] + betaeps.cc[2]*popden[i] + betaeps.cc[3]*elevation[i]
    + randomeps.cc[area[i]]
  }
}
...
for (i in 1:nsite){
  for (s in 2:nprimary){
    ...
    + betagamma.cl[3,z.cc[site.cc[i],pocc.cc[i]]  +1]*elevation[i] + randomgamma.cl[z.cc[site.cc[i],pocc.cc[i]]+1,area[i]]
    ...
    + betaeps.cl[3,z.cc[site.cc[i],pocc.cc[i]]+1]*elevation[i] + randomeps.cl[z.cc[site.cc[i],pocc.cc[i]]+1, area[i]]
  }
}
...
for (i in 1:nobs.cl){
  ...
  pstarrep.cl[i] <- 1-(1-p.cl[i])^ndays.cl[i]
  yrep.cl[i] ~ dbern(z.cl[site.cl[i],pocc.cl[i]]*pstarrep.cl[i])
}
...
# to run the model
na <- 100000 ; ni <- 300000; nt <- 200 ; nb <- 100000 ; nc <- 3
...
cctocl_dynamic_add_072023_gof <- jags(jags.data, inits, params, "cctocl_dynamic_add_072023_gof.txt", n.chains = nc, n.adapt = na, n.iter = ni, n.burnin = nb, n.thin = nt, parallel = T)

end <- Sys.time()

end - start

print(cctocl_dynamic_add_072023_gof)

str(cctocl_dynamic_add_072023_gof$sims.list)
...
for (i in 1:cctocl_dynamic_add_072023_gof$mcmc.info$n.samples){
  yrep.cl[[i]] <- cctocl_dynamic_add_072023_gof$sims.list$yrep.cl[i,]
...
for (i in 1:cctocl_dynamic_add_072023_gof$mcmc.info$n.samples){
  p_model[[i]] <- cctocl_dynamic_add_072023_gof$sims.list$pstar[i,]
...
y3drep <- vector(mode = 'list', length = cctocl_dynamic_add_072023_gof$mcmc.info$n.samples)

for (k in 1:cctocl_dynamic_add_072023_gof$mcmc.info$n.samples){
...
p_est <- vector(mode = 'list', length = cctocl_dynamic_add_072023_gof$mcmc.info$n.samples)
...
for (i in 1:cctocl_dynamic_add_072023_gof$mcmc.info$n.samples){
  psi.cl[[i]] <- cctocl_dynamic_add_072023_gof$sims.list$psi.cl[i,,]
  gamma.cl[[i]] <- cctocl_dynamic_add_072023_gof$sims.list$gamma.cl[i,,]
  eps.cl[[i]] <- cctocl_dynamic_add_072023_gof$sims.list$eps.cl[i,,]

...
# This is the code for WINTER season
...
setwd("/path_to_files/")
load("path_to_data/cctocl_static_winter_data_github.RData") # load prepared jags data
...
cat(file = "cctocl_static_winter_072023.txt"," 
model {

# Model: cctocl_static_winter_072023
#
...
for (i in 1:nsite){
  DW_W[i] <- z.cc[i] * z.cl[i]
  Dw_W[i] <- z.cc[i] * (1-z.cl[i])
  dW_W[i] <- (1-z.cc[i]) * z.cl[i]
  dw_W[i] <- (1-z.cc[i]) * (1-z.cl[i])
}

nDW_W <- sum(DW_W[])        # Number of sites with both species present

nDw_W <- sum(Dw_W[])        # Number of sites with only roe deer present

ndW_W <- sum(dW_W[])        # Number of sites with only gray wolf present

ndw_W <- sum(dw_W[])        # Number of sites with both species absent
...
cctocl_static_winter_072023 <- jags(jags.data, inits, params, "cctocl_static_winter_072023.txt", n.chains = nc, n.adapt = na, n.iter = ni, n.burnin = nb, n.thin = nt, parallel = T)

end <- Sys.time()

end - start

print(cctocl_static_winter_072023)
